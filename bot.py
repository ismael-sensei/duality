...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):
        extensions = [f"adapter.controllers.{filename[:-3]}" for filename in os.listdir(f"{os.path.realpath(os.path.dirname(__file__))}/adapter/controllers") if filename.endswith("_cog.py")]
